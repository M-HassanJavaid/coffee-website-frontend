...
const Navbar = ({animation}) => {
  ...
  return (
    <nav className={`${animation ? 'bg-transparent' : 'bg-neutral-800'} fixed z-10 top-0 left-0 w-screen`} ref={navbar}>
      <div className='flex relative gap-2.5 justify-between px-5 py-3 h-[80px] items-center text-zinc-50 max-w-[1700px] mx-auto'>
        ...
        <ul className={`flex text-nowrap gap-4 overflow-hidden transition-all max-lg:absolute max-lg:top-full max-lg:left-0 max-lg:w-full max-lg:bg-neutral-800 max-lg:px-5 
          ...
          <li className='block my-1.5 hover:text-amber-400'><Link to='/'>MENU</Link></li>
          <li className='block my-1.5 hover:text-amber-400'><Link to='/'>ABOUT US</Link></li>
          <li className='block my-1.5 hover:text-amber-400'><Link to='/'>CONTACT US</Link></li>
        ...
        <ul className='flex gap-4 items-center max-lg:order-3'>
          <li className='hover:text-amber-400 hover:scale-110'><FontAwesomeIcon icon={faCartShopping} className='text-2xl cursor-pointer' /></li>
          <li className='hover:text-amber-400 hover:scale-110'><FontAwesomeIcon icon={faUser} className='text-2xl cursor-pointer' /></li>
        </ul>
      </div>
    </nav>
  )
